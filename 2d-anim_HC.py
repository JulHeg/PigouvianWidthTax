...
This the backup copy of 2d-anim2.py. It doesn't have a live camera feed and doesn't actually 3D scan anything. But it still has the nice UI if our hardware goes bad.
@author: Daniel
"""
...
while running:
    ...
    try:
        keys = pg.key.get_pressed()
        if keys[pg.K_SPACE]:
            txt_input = [5, 7.5, 12.2][car_counter]
            # wait 1 second before starting the next car
            if time.time() - start_times[car_counter] > 1:

                #old_time = txt_input[1]
                new_x_coordinate_list = make_new_coordinate_list(float(txt_input), colist, car_counter, dpi)
                car_counter += 1
                # update width and price (= 0.4 * width)q
                print('Ttext changed')
                texts[1] = font.render(str(round(float(txt_input), 2)) + " cm", True, text_color)
                texts[3] = font.render("€" + str(round(float(txt_input) * 0.4,2)) + "", True, text_color)
